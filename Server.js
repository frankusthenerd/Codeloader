...
function Create_Folder(params, response) {
  ...
  try {
    var dest = path.join($startup, folder);
    fs.mkdirSync(dest, {
      recursive: true
    });
  ...
  if (request.method == "GET") {
    ...
    if (file.match(/\w+\.\w+$/)) {
      ...
      if ($config["home"] != undefined) {
        Read_File($config["home"] + ".html", response, params);
      }
      else { 
...
function Launch_Window() {
  if (($config["browsers"] != undefined) && ($config["cmd-params"] != undefined) && ($config["command"] != undefined)) {
    var browsers = $config["browsers"].split(/,/);
    var browser_count = browsers.length;
    for (var browser_index = 0; browser_index < browser_count; browser_index++) {
      var browser = browsers[browser_index];
      try {
        var params = $config["cmd-params"].split(/,/);
        child_process.execFile($config["command"], params.concat(browser, [ "http://localhost:" + $config["port"] ]));
        break;
      }
      catch (error) {
...
function Load_Config(name) {
  try {
    var data = fs.readFileSync(path.join($startup, name + ".txt"), "utf8");
...
function Get_Local_Path(folder) {
  var folders = $startup.split(path.sep).concat(folder.split(path.sep));
  var new_folders = [];
 ...
 * @param params The parameter object containing the command.
 * @param response The server response.
 */
function Run_Command(params, response) {
  if (params.command != undefined) {
    try {
      var code = (params.code == undefined) ? "" : params.code;
      Validate_User(code, "", "none");
      var command = params.command.split(/\s+/);
      if (command.length > 0) {
        var cmd = child_process.spawn(NODE, command, {
          cwd: $root
        });
        var data = "";
        ...
        response.end("Command Error: Command not complete!");
      }
    }
    catch (error) {
      response.writeHead(401, {
      ...
      response.end("Command Error: " + error.toString());
    }
  }
  else {
    response.writeHead(401, {
    ...
    response.end("Command Error: You need to enter a command.");
  }
}
 ...
 * @throws An error if the user code is not valid.
 */
function Validate_User(code, file, mode) {
  var found = false;
  for (var key in $keys) {
    var access = $keys[key];
    if (code == key) {
      if (mode != "none") { // We don't need to validate permissions.
        Validate_Permissions(file, access, mode);
      }
      found = true;
      break;
    }
  ...
  if (!found) {
    throw String("User code is not valid.");
  }
}
 ...
 * @param access 
 */
function Validate_Permissions(file, access, mode) {
  var found = false;
  var pattern_count = $patterns.length;
  for (var pat_index = 0; pat_index < pattern_count; pat_index++) {
    var fentry = $patterns[pat_index]; // We need to keep ordering here.
    var file_access = parseInt($permissions[fentry]);
    var fmode = fentry.substr(0, 1);
    var fpattern = fentry.substr(1);
    if ((access >= file_access) && (mode == fmode) && (file.match(new RegExp(fpattern, "")))) {
      found = true;
      break;
    }
  }
  if (!found) {
    throw String("Cannot access " + file + " - invalid permissions.");
  }
}

/**
 * Checks to see if a file needs to be validated.
 * @param file The file to test for needing validation.
 * @param mode The file mode.
 * @return True if file needs validation, false otherwise.
 */
function File_Needs_Validation(file, mode) {
  var found = false;
  var pattern_count = $patterns.length;
  for (var pat_index = 0; pat_index < pattern_count; pat_index++) {
    var fentry = $patterns[pat_index]; // We need to keep ordering here.
    ...
    var fmode = fentry.substr(0, 1);
    var fpattern = fentry.substr(1);
    if ((mode == fmode) && (file.match(new RegExp(fpattern, "")))) {
      found = (file_access > 0);
      break;
    }
  }
...
function Load_Keys(name) {
  try {
    ...
    for (var rec_index = 0; rec_index < rec_count; rec_index++) {
      var record = records[rec_index].split(/=/);
      if (record.length == 2) {
        var user_code = record[0];
        if (!isNaN(record[1])) {
          var access = parseInt(record[1]);
          $keys[user_code] = access;
        }
      }
    }
  }
  catch (error) {
    Debug(name, error.toString());
  }
}
 ...
 * @param name The name of the file.
 */
function Load_Permissions(name) {
  try {
    var data = fs.readFileSync(path.join($root, name), "utf8");
    var records = Split(data);
    var rec_count = records.length;
    for (var rec_index = 0; rec_index < rec_count; rec_index++) {
      var record = records[rec_index].split(/=/);
      if (record.length == 2) {
        var permission = record[0];
        if (!isNaN(record[1])) {
          var access = parseInt(record[1]);
          $permissions[permission] = access;
          // Also record in pattern list.
...
function Init() {
  try {
    ...
    server_instance.on("listening", function() {
      Launch_Window(); // Open game window.
    });
  }
  catch (error) {
    console.log("Error: " + error.message);
